...
const toBand = (value: number): CoachConfidenceBand => {
  if (value >= 70) return "high";
  if (value >= 40) return "medium";
  return "low";
};

const getDayDiff = (dateInput?: Date | string | null) => {
...
const getRecentVelocity = (goal: CoachGoal) => {
  const windowStart = Date.now() - DATA_WINDOW_DAYS * MS_IN_DAY;
  const recentEvents = (goal.progressEvents ?? []).filter(
    (event) => new Date(event.createdAt).getTime() >= windowStart
  );
...
const getArchetype = (
  ...
  if ((dueDays !== null && dueDays < 0) || staleDays >= 7) return "rescue";
  if (goal.currentProgress >= 80 || (goal.currentProgress >= 65 && velocity >= 8)) return "close";
  return "push";
};

const getRiskSeverity = (score: number): CoachRiskSeverity => {
...
const getPriorityReason = (
  archetype: "rescue" | "push" | "close",
  dueDays: number | null,
  staleDays: number,
  velocity: number
) => {
  if (archetype === "rescue") {
    if (dueDays !== null && dueDays < 0) return "Overdue goal: recover immediately with a focused execution block.";
    if (staleDays >= 7) return "Momentum dropped for 7+ days; immediate reactivation needed.";
...
export const buildRulesSummary = (goals: CoachGoal[]): CoachSummary => {
  ...
  const scored = activeGoals.map((goal) => {
    ...
    const urgencyScore =
      (dueDays !== null && dueDays < 0 ? 100 : dueDays !== null && dueDays <= 7 ? 70 : dueDays === null ? 35 : 20) +
    ...
    const effortProxyScore =
      goal.currentProgress >= 80 && staleDays >= 3
        ? 90
  ...
  const risks: CoachRiskItem[] = sortedByPriority
    .filter((item) => {
  ...
  const nextActions: CoachActionItem[] = [];
  for (const action of actionCandidates) {
    if (seenGoals.has(action.goalId)) continue;
  ...
  const cadenceComponent = clamp(
    100 -
      Math.round(
        (scored.reduce((sum, item) => sum + item.staleDays, 0) / Math.max(scored.length, 1)) * 7
      ),
    0,
    100
  );
  const deadlineComponent = clamp(
    100 - Math.round((overdueCount / Math.max(scored.length, 1)) * 100),
    0,
    100
  );
  const completionComponent = clamp(
    Math.round(
      scored.reduce((sum, item) => sum + item.feasibilityScore, 0) / Math.max(scored.length, 1)
    ),
    0,
    100
  );
  ...
  const confidenceValue = Math.round(
    clamp(cadenceComponent * 0.35 + deadlineComponent * 0.4 + completionComponent * 0.25 - staleCount * 2, 0, 100)
  );
