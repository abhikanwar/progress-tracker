...
import { generateCoachChatReply, generateCoachChatReplyStream, rewriteSummaryWithAi, } from "./coach.ai.js";
...
const parseRelativeTargetDate = (message) => {
    const match = message.match(/\bin\s+(\d{1,3})\s*(day|days|week|weeks|month|months)\b/i);
    if (!match)
        return undefined;
...
const normalizeGoalTitle = (value) => {
    return value
        .trim()
        .replace(/^["']+|["']+$/g, "")
        .replace(/\s+/g, " ")
...
const extractQuoted = (value) => {
    const match = value.match(/["']([^"']{2,})["']/);
    return match?.[1]?.trim() ?? null;
};
const parseFollowUpIntent = (message, history) => {
    ...
    for (let index = history.length - 1; index >= 0; index -= 1) {
        ...
        if (content.includes("[intent:delete_goal]"))
            return "delete_goal";
        if (content.includes("[intent:update_goal]"))
            return "update_goal";
        if (content.includes("i can delete that goal"))
            return "delete_goal";
        if (content.includes("i can update that goal"))
            return "update_goal";
        break;
    }
    return null;
};
const maybeBuildCreateProposal = (message) => {
    const text = stripPolitePrefix(message);
    const lowered = normalize(text);
    const hasIntent = /\b(create|add|start|set up|setup)\b/.test(lowered) &&
...
const maybeBuildDeleteProposal = (message, goals, forceIntent = false) => {
    const text = stripPolitePrefix(message);
    const lowered = normalize(text);
    const hasIntent = forceIntent || (/\b(delete|remove)\b/.test(lowered) && /\bgoal\b/.test(lowered));
    if (!hasIntent)
    ...
    if (matchingGoals.length !== 1) {
        const suggestions = goals
            ...
            .slice(0, 5)
            .map((goal) => `- ${goal.title}`);
        const ambiguityMessage = suggestions.length > 0
...
const maybeBuildUpdateProposal = (message, goals, forceIntent = false) => {
    const text = stripPolitePrefix(message);
    const lowered = normalize(text);
    const hasIntent = forceIntent || /\b(update|edit|rename|change)\b/.test(lowered);
    if (!hasIntent)
        return { proposal: null };
    const quotedParts = [...text.matchAll(/["']([^"']{2,})["']/g)].map((match) => match[1].trim());
    ...
    if (!targetTitle) {
        ...
        if (mentionedGoals.length === 1) {
            ...
            return {
                proposal: null,
                ambiguityMessage: `I can update that goal, but I found multiple matches. Reply with the exact title in quotes and what to change.\n${suggestions.join("\n")}\n[intent:update_goal]`,
            };
        }
    }
    ...
    const titleFromRenamePattern = text.match(/\b(?:rename|title)\s*(?:to|=|:)?\s*["']?([^"';,\n]{2,120})["']?/i)?.[1]?.trim() ??
        text.match(/\b(?:change)\b.+\bto\b\s+["']?([^"';,\n]{2,120})["']?/i)?.[1]?.trim() ??
        "";
    const details = text.match(/\b(?:details?|description|notes?)\s*(?:to|=|:)\s*["']?(.+?)["']?(?=(?:\s+\b(?:target\s*date|due|rename|title)\b|$))/i)?.[1]?.trim() ?? undefined;
    const targetDateRaw = text.match(/\btarget\s*date\s*(?:to|=|:)?\s*(\d{4}-\d{2}-\d{2})/i)?.[1] ??
        undefined;
    const dueDateRaw = text.match(/\bdue\s*(?:on|by)?\s*(\d{4}-\d{2}-\d{2})/i)?.[1] ?? undefined;
    const relativeDateFragment = text.match(/\b(?:target\s*date|due)\s*(?:to|=|:|on|by)?\s*(in\s+\d{1,3}\s*(?:day|days|week|weeks|month|months))/i)?.[1] ??
        undefined;
    ...
    if (updates.title === undefined &&
        ...
        return {
            proposal: null,
            ambiguityMessage: `I found "${matchingGoals[0].title}", but I need the change details. Example: "Update goal \\"${matchingGoals[0].title}\\" to \\"New Title\\"".\n[intent:update_goal]`,
        };
    }
    return {
        proposal: {
...
const removeIntentMarkers = (value) => value.replace(/\n?\[intent:(delete_goal|update_goal)\]\s*/gi, "").trim();
const buildFallbackReply = (goals) => {
...
const prepareChatContext = async (userId, input) => {
    ...
    const deleteProposalResult = maybeBuildDeleteProposal(message, goalCandidates, followUpIntent === "delete_goal");
    const updateProposalResult = maybeBuildUpdateProposal(message, goalCandidates, followUpIntent === "update_goal");
    const proposedActions = [createProposal, updateProposalResult.proposal, deleteProposalResult.proposal].filter((proposal) => proposal !== null);
    const ambiguityMessage = deleteProposalResult.ambiguityMessage ?? updateProposalResult.ambiguityMessage;
    return {
        message,
        targetConversation,
        userMessage,
...
const persistAssistantReply = async (userId, input) => {
...
export const coachService = {
    ...
    sendChatMessage: async (userId, input) => {
        ...
        const updatedConversation = (await coachRepo.getConversationById(userId, context.targetConversation.id)) ??
    ...
    sendChatMessageStream: async (userId, input, onToken) => {
        const context = await prepareChatContext(userId, input);
    ...
    executeChatAction: async (userId, proposalId, confirmText) => {
        try {
    ...
    undoChatAction: async (userId, proposalId) => {
        try {
